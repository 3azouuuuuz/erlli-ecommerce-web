...
const DeliverySection = ({ productId, selectedShippingOption, onShippingOptionChange }) => {
      ...
      try {
        ...
        if (customData) {
          allOptions = customData.map(opt => ({
            id: opt.id,
            name: opt.name,
            duration: t('DeliveryDays', { min: opt.min_days, max: opt.max_days }),
            returnDuration: opt.return_min_days && opt.return_max_days
              ? t('DeliveryDays', { min: opt.return_min_days, max: opt.return_max_days })
              : t('NotAvailable'),
            price: opt.price === 0 ? t('FreeShipping') : `$${opt.price.toFixed(2)}`,
            priceNum: opt.price,
            icon: '🚚',
          }));
        }
  ...
  }, [productId, selectedShippingOption, onShippingOptionChange, t]);
  ...
  return (
    <DeliveryContainer>
      <SectionHeader>
        ...
        <SectionTitle>{t('ShippingOptions')}</SectionTitle>
      </SectionHeader>
      {loading ? (
        <LoadingSpinner />
          ...
          {shippingOptions.map((option) => (
            ...
            >
              ...
              <OptionInfo>
                ...
                <OptionDetails>
                  <DetailRow>📦 {t('Delivery')}: {option.duration}</DetailRow>
                  <DetailRow>↩️ {t('Returns')}: {option.returnDuration}</DetailRow>
                </OptionDetails>
              </OptionInfo>
              <OptionPrice free={option.priceNum === 0}>
                {option.price}
              </OptionPrice>
            </OptionCard>
          ))}
        ...
        <NoOptionsText>{t('NoShippingOptionsAvailable')}</NoOptionsText>
      )}
    </DeliveryContainer>
  );
};
