...
const VendorSettings = () => {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      // Delete vendor-related data (stores and products)
      await supabase.from('stores').delete().eq('vendor_id', profile.id);
      await supabase.from('products').delete().eq('vendor_id', profile.id);
  ...
  <PageContainer>
    <Container>
      <Header>
        <PageTitle>{t('VendorSettings')}</PageTitle>
      </Header>
      <ContentWrapper>
        <Section>
          <SectionTitle>{t('VendorProfile')}</SectionTitle>
          <SettingsBox onClick={() => navigate('/vendor/profile-settings', { state: { fromVendor: true } })}>
            <SettingsLabel>{t('VendorProfile')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
          <SettingsBox onClick={() => navigate('/vendor/security-settings')}>
            <SettingsLabel>{t('Security')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
          <SettingsBox onClick={() => navigate('/vendor/store-settings')}>
            <SettingsLabel>{t('StoreSettings')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
        </Section>
        <Section>
          <SectionTitle>{t('ShopSettings')}</SectionTitle>
          <SettingsBox onClick={() => navigate('/vendor/country')}>
          ...
          <SettingsBox onClick={() => navigate('/vendor/currency')}>
            <SettingsLabel>{t('Currency')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
          <SettingsBox onClick={() => navigate('/vendor/payout-settings')}>
            <SettingsLabel>{t('PayoutSettings')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
          <SettingsBox onClick={() => navigate('/vendor/subscription-settings')}>
            <SettingsLabel>{t('SubscriptionSettings')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
        </Section>
        <Section>
          <SectionTitle>{t('Account')}</SectionTitle>
          <SettingsBox onClick={() => navigate('/vendor/Lang')}>
            <SettingsLabel>{t('Language')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
          <SettingsBox onClick={() => navigate('/vendor/About')}>
            <SettingsLabel>{t('About')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
          <SettingsBox onClick={handleLogout}>
            <SettingsLabel danger>{t('Logout')}</SettingsLabel>
            <ArrowIcon />
          </SettingsBox>
          <DeleteButton onClick={() => setIsDeleteModalVisible(true)}>
            <DeleteText>{t('DeleteVendorAccount')}</DeleteText>
          </DeleteButton>
        </Section>
      </ContentWrapper>
      <Spacer />
